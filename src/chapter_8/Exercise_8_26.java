 ...
 * 		0.30 0.12 0.4
 * 
 * 		The row-sorted array is 0.15 0.375 0.875 0.005 0.225 0.55 0.12 0.30 0.4
 * 		0.15 0.375 0.875
 * 		0.005 0.225 0.55
 * 		0.12 0.30 0.4
 * 
...
/** Creating the class named Exercise_8_26 */
public class Exercise_8_26{
	...
	public static void main(String[] arg) {
		...
		System.out.print("\n\tPrinting  matrix before sorting rows");
		/** Printed the matrix before sorted rows */
		printMatrix(matrix);

		/** Called the method sortRow in order to sort the row of the matrix  */
	...
	/** Created a method following the initial instructions of a sorting row matrix exercise */
		...
		for (int row = 0; row < m.length; row++) {
			...
			for(int columns = 0; columns < m[row].length; columns++) {
				...
				for(int extra = 0; extra < m.length;extra++) {
					...
					if( m[row][columns] < m[row][extra]) {
						//created a temporary variable in order to swipe the numbers
						double temp = m[row][columns];
						m[row][columns] = m[row][extra];
						m[row][extra] = temp;
